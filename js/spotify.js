...
export async function buscarPlaylist(emocion, token) {
  const resp = await fetch(
    `https://api.spotify.com/v1/search?q=${emocion}&type=playlist&limit=20`,
    {
      headers: {
  ...
  if (!data.playlists || !Array.isArray(data.playlists.items) || data.playlists.items.length === 0) {
    throw new Error(`No se encontró ninguna playlist para: ${emocion}`);
  }
